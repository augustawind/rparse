    ...
    P1: Parser<Stream = S>,
{
    ...
    fn parse_lazy(&mut self, stream: Self::Stream) -> ParseResult<Self::Stream, Self::Output> {
        match self.0.parse_partial(stream).as_tuple() {
            (Ok(Some(r0)), stream) => match self.1.parse_partial(stream).as_tuple() {
    ...
    P2: Parser<Stream = S>,
{
    ...
    fn parse_lazy(&mut self, stream: Self::Stream) -> ParseResult<Self::Stream, Self::Output> {
        match self.0.parse_partial(stream).as_tuple() {
            (Ok(Some(r0)), stream) => match self.1.parse_partial(stream).as_tuple() {
                (Ok(Some(r1)), stream) => match self.2.parse_partial(stream).as_tuple() {
